...
var s,d,r,g;
e=e||{};var k=parseInt(e.startSlide,10)||0;
var v=e.speed||300;
...
if(B.transitions){C.style.left=(D*-r)+"px";
q(D,k>D?-r:(k<D?r:0),0);
...
j(k*-r,G*-r,D||v);
}k=G;u(e.callback&&e.callback(k,s[k]));
}function q(C,E,D){l(C,E,D);
d[C]=E;}function l(D,G,F){var C=s[D];
var E=C&&C.style;
if(!E){return;
...
return;}var E=+new Date;
var D=setInterval(function(){var H=+new Date-E;
if(H>C){c.style.left=F+"px";
if(A){x();}e.transitionEnd&&e.transitionEnd.call(event,k,s[k]);
clearInterval(D);
return;}c.style.left=(((F-G)*(Math.floor((H/C)*100)/100))+G)+"px";
},4);}var A=e.auto||0;
var w;function x(){w=setTimeout(p,A);
}function t(){A=0;
clearTimeout(w);
}var h={};var y={};
var z;var b={handleEvent:function(C){switch(C.type){case"touchstart":this.start(C);
break;case"touchmove":this.move(C);
break;case"touchend":u(this.end(C));
break;case"webkitTransitionEnd":case"msTransitionEnd":case"oTransitionEnd":case"otransitionend":case"transitionend":u(this.transitionEnd(C));
break;case"resize":u(n);
break;}if(e.stopPropagation){C.stopPropagation();
}},start:function(C){var D=C.touches[0];
h={x:D.pageX,y:D.pageY,time:+new Date};
z=undefined;
y={};c.addEventListener("touchmove",this,false);
...
t();if(e.continuous){l(i(k-1),y.x+d[i(k-1)],0);
l(k,y.x+d[k],0);
...
q(i(k+1),d[i(k+1)]-r,v);
k=i(k+1);}else{if(e.continuous){q(i(k+1),r,0);
q(i(k-2),-r,0);
}else{q(k+1,r,0);
}q(k,d[k]+r,v);
q(i(k-1),d[i(k-1)]+r,v);
k=i(k-1);}e.callback&&e.callback(k,s[k]);
