 ...
 * 2015-09-02
 */
 class DoctorController extends Q_Controller_Smarty{
 	...
 	public function updatepasswdAction(){
 		...
 		if ($userInfo != null) {
 			...
 			if ($intRow > 0) {
 				//跳转到登录界面
 				$url = "http://wapask.9939.com/login";
 				$this->_redirect($url);
 			}
 		}
 	...
 	public function detailmsgAction() {
 		...
 		$md5URL = md5($_SERVER['REQUEST_URI']);
 		echo $this->view->render("doctor/detailmsg.tpl", $md5URL);
 	}
 	...
 	public function updateuserdetailAction(){
 		...
 		$user = array('nickname' => $userDetail['nickname']);
 		$uid = $userDetail['uid'];
 		...
 		if (!empty($updateMemberDetail)) {
 			...
 			if (!empty($updateUser)) {
 				$url = "http://wapask.9939.com/doctor/usercenter?userid=" . $uid;
 				$this->_redirect($url);
 			}
 		}
 	...
 	private function getUserDetail($request) {
 		...
 		$userDetail['telephone'] = $this->htmlFilter->filter($initTelephone);
 	...
 	public function moredoctorAction() {
 		...
 		echo $this->view->render('askdoctor/moredoctor.tpl');
 	} 	
 	...
 	public function moredoctorareaAction() {
 		...
 		echo $this->view->render('askdoctor/moredoctorArea.tpl');
 	} 	
 	...
 	public function showmoredoctorpageAction(){
 		...
 		$md5URL = md5($_SERVER['REQUEST_URI']);
 		echo $this->view->render('askdoctor/doctor.tpl', $md5URL);
 	}
 	...
 	public function doctorabstractAction(){
 		...
 		$md5URL = md5($_SERVER['REQUEST_URI']);
 		echo $this->view->render("doctor/abstract.tpl", $md5URL);
 	}
 	...
 	public function doctorgoodatAction(){
 		...
 		$md5URL = md5($_SERVER['REQUEST_URI']);
 		echo $this->view->render("doctor/goodAt.tpl", $md5URL);
 	}
 	...
 	public function indexAction() {
 		...
 		$md5URL = md5($_SERVER['REQUEST_URI']);
 		echo $this->view->render("doctor/doctor.tpl", $md5URL);
 	}
 	...
 	public function detaildoctorpageAction() {
 		...
 		$md5URL = md5($_SERVER['REQUEST_URI']);
 		echo $this->view->render("doctor/doctorPage.tpl", $md5URL);
 	}
