 ...
 * 2015年9月28日
 */
class LoginController extends Q_Controller_Smarty{
	...
	public function registerAction(){
		 ...
		 * 1、获取到用户注册的信息集
		 * 2、验证当前注册的用户名是否存在
		 * 	2.1、如果存在，则跳转到注册的页面，进行重新注册
		 * 	2.2、如果不存在，则进行注册操作
		 * 		2.2.1、注册成功，跳转到【用户个人中心】
		 * 		2.2.2、注册失败，则跳转到注册页面，进行重新注册操作
		 */
		$renderTPL = "login/register.tpl";
		...
		//2、验证当前注册的用户名是否存在
		$userRow = $this->member->checkUserExist(0, $user['username'], "", true);
		if (!empty($userRow)) {
			
			//2.1、如果存在，则跳转到注册的页面，进行重新注册
			$this->view->assign("error", "用户名存在");
		}else{
			unset($user['initPassword']);
			//2.2、如果不存在，则进行注册操作
			...
			if (!empty($userid)) {
				...
				$this->view->assign("error", "注册失败，请重试！");
			}
		}
		//将注册的信息，回显到页面中
		$this->view->assign("username",$user['username']);
		$this->view->assign("nickname",$user['nickname']);
		$this->view->assign("email",$user['email']);
		$this->view->assign("password",$user['initPassword']);
	...
	public function loginAction(){
		...
		if (isset($userInfo) && !empty($userInfo)) {
			...
			//设置用户 userid 到 session 中
			Zend_Session::start();
			$authNamespace = new Zend_Session_Namespace('userid'.$userInfo['uid']);
			$authNamespace->setExpirationSeconds(2*7*24*60*60);
			$key = "userid".$userInfo['uid'];
			if (!isset($authNamespace->$key)) {
				$authNamespace->$key = $userInfo['uid'];
				//$authNamespace->setExpirationSeconds(2*7*24*60*60);
			}
			
			//2.1、如果存在登录的用户，则跳转到【用户个人中心】
			$url = "http://wapask.9939.com/doctor/usercenter?userid=" . $userInfo['uid'] . "&login=" . $login;
			$this->_redirect($url);
		}
		$this->view->assign("error", "登录信息有误！");
	...
	public function indexAction() {
		echo $this->view->render("login/login.tpl");
	}
