 ...
 */
 class App_Model_Memberfriend extends App_BaseTable{
 	...
 	public function getDoctorFriends($uid, $start, $pageSize) {
 		...
 		$innerSQL = " SELECT fuid FROM member_friend WHERE status != 2 AND uid = ?  ";
 		$sql = " SELECT uid, nickname, pic FROM member where uid in (". $innerSQL .") AND  uType = 2 AND status = 1 LIMIT " . $start . ", " . $pageSize ;
