...
class App_Model_Zxads extends App_BaseTable {
    ...
    private function default_ads_pic($ads){
        extract($ads);
        if(stripos($imageurl, self::PCDOMAIN)===false){
            $imageurl = self::PCDOMAIN . 'uploadfile/' . $imageurl;
        }
        $ret = '';
    ...
    private function default_ads_ref($ads){
        extract($ads);
        $ret = str_replace('http://union.9939.com/cpro/ui/cm.js', 'http://cpro.baidustatic.com/cpro/ui/cm.js', $text);
        return $ret;
    }
