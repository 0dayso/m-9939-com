 ...
 * ==============================================
 */
class App_Model_KeyWords extends QModels_Article_Table {
    ...
    public function list_one($id) {
        ...
        $result = $this->db_v2_read->fetchRow($sql); //获取一行
        return $result;
    }
    ...
    public function GetCount($where = '') {
        ...
        $result = $this->db_v2_read->fetchRow($sql);
        return $result['num'];
    }
    ...
    public static function createCacheRandWords(array $condition = array(),$expired=24){
        ...
        $rand_words_save_path = $save_root_path.DIRECTORY_SEPARATOR.'caches/randwords'.'_'.$key;
        //拼音首字母页码
    ...
    public static function getCacheRandWords($size = 100, array $condition = array()) {
        ...
        $key = md5(json_encode($condition));
        $save_root_path = 'rand_words';
        $cache_key = sprintf('%s|%s|%s', 'caches', 'randwords'.'_'.$key, 'words');
